...
class _WordleRowState extends State<WordleRow> {
  ...
  Widget build(BuildContext context) {
    return Container(
      height: widget.enabled ? 110 : 54,
      width: widget.enabled ? (widget.length + 1) * 54 : widget.length * 54,
      alignment: Alignment.center,
      child: Padding(
        padding:
            widget.enabled ? const EdgeInsets.only(left: 25) : EdgeInsets.zero,
        child: ListView.builder(
          scrollDirection: Axis.horizontal,
          itemCount: widget.enabled ? widget.length + 1 : widget.length,
